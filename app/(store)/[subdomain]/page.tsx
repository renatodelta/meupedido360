        ...
        <div className="space-y-12">
            ...
            return (
              <section key={category.id} id={category.id} className="scroll-mt-36 space-y-6">
                ...
                {categoryProducts.length === 0 ? (
                  <div className="py-6 px-4 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 text-center text-slate-400 text-xs">
          ...
          {(() => {
            const uncategorized = displayProducts.filter(
              (p) => !displayCategories.some((c) => c.id === p.category_id)
            );
            if (uncategorized.length === 0) return null;
            return (
              <section id="outros" className="scroll-mt-36 space-y-6">
                <div className="flex items-center gap-3">
                  <h2 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">
                    Geral / Outros Itens
                  </h2>
                  <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 font-medium">
                    {uncategorized.length} {uncategorized.length === 1 ? 'item' : 'itens'}
                  </span>
                  <div className="h-0.5 flex-1 bg-slate-100 dark:bg-slate-800" />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
